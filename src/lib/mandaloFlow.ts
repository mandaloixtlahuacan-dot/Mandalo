...
// Qué tienda usar para inyectar su menú de precios fijos al prompt. NO se
// limita a tiendas abiertas — el cliente puede querer ver el menú de una
// cerrada para programar su pedido. Orden: (1) la tienda ya elegida en el
// snapshot; (2) alguna tienda (abierta o cerrada) nombrada en el mensaje o
// los últimos turnos, incluidos los del bot ("Solo hay X para comida
// preparada"); (3) si hay una sola tienda activa; (4) si hay exactamente una
// tienda de catálogo — es la más probable cuando no hay otra señal.
function resolveMenuCandidatoTienda(params: {
  currentOrderState: JsonObject;
  activeTiendas: TiendaRow[];
  userMessage: string;
  ...
  if (Number.isFinite(snapId) && snapId > 0) {
    return params.activeTiendas.find((t) => Number(t.id) === snapId) ?? null;
  }

  const textos = [params.userMessage, ...params.historial.slice(-4).map((m) => m.content)].join(" ").toLowerCase();
  for (const t of params.activeTiendas) {
    const nombre = String(t?.nombre ?? "").toLowerCase().trim();
    if (!nombre) continue;
    if (textos.includes(nombre)) return t;
    const palabras = nombre.split(/\s+/).filter((w) => w.length > 3);
    if (palabras.some((w) => textos.includes(w))) return t;
  }

  if (params.activeTiendas.length === 1) return params.activeTiendas[0];

  const catalogo = params.activeTiendas.filter((t) => t.usa_catalogo_fijo === true);
...
export async function getLLMResponse(params: {
  ...
  const horarioTexto = (n: TiendaRow): string =>
    describeHorarioTienda({
  ...
  const tiendas_text = tiendas.length
    ? tiendas
        .map((n) => {
          const horario = horarioTexto(n);
          return `- ${String(n.nombre ?? "")}${n.categoria ? ` (${String(n.categoria)})` : ""} [${String(n.telefono ?? "")}]${horario ? ` — horario: ${horario}` : ""}`;
        })
        .join("\n")
  ...
  const tiendasCerradas_text = tiendasCerradas.length
    ? tiendasCerradas
        .map((n) => {
          ...
          const horario = horarioTexto(n);
          return `- ${String(n.nombre ?? "")}${n.categoria ? ` (${String(n.categoria)})` : ""} — cerrada ahora${abre}${horario ? `; horario: ${horario}` : ""}`;
        })
        .join("\n")
    : "(ninguna)";
  ...
  const menuCandidatoId = Number(menuCandidato?.id);
  if (menuCandidato?.usa_catalogo_fijo === true && Number.isFinite(menuCandidatoId) && menuCandidatoId > 0) {
    try {
